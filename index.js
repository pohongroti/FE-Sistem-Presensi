...
app.listen(port, () => {
  console.log(
    'Sistem Presensi Pegawai v1.0.0 berjalan pada server Localhost di port 3500'
  );
});
...
exports.executeQuery = function (query, callback) {
  connectionKeKolam.getConnection(function (err, connection) {
    if (err) {
      ...
      throw err;
      console.log(
        'Sistem Presensi Pegawai v1.0.0 Sudah Terhubung ke DBMS MySQL',
        res
      );
    }
...
app.post('/user/login', (req, res) => {
  ...
  if (idpegawai && sandi) {
    connectionKeKolam.query(
      `SELECT * FROM pegawai WHERE idpegawai="${idpegawai}" AND sandi="${sandi}"`,
      (err, results) => {
        if (err) throw err;
        if (results.length) {
...
app.get('/user/presensimasuk/:idpegawai', (req, res) => {
  ...
  connectionKeKolam.query(
    `SELECT pegawai.idpegawai, pegawai.namapegawai, pegawai.jabatanpegawai FROM pegawai WHERE idpegawai="${req.session.idpegawai}"`,
    (err, results) => {
      if (err) throw err;
      const pegawaiData = results.filter(function (pegawai) {
        return pegawai.idpegawai === req.session.idpegawai;
      });
      return res.render('presensimasuk', {
        idpegawai: req.session.idpegawai,
        pegawai: results,
        pegawaiData: pegawaiData[0],
      });
    }
  );
});
...
app.post('/user/addpresensimasuk', (req, res) => {
  ...
  connectionKeKolam.query(
    `INSERT INTO presensi (idpresensi,idpegawai,tglwaktu,longitude,latitude,jenis_presensi,sistem_kerja,foto) VALUES ("${idpresensi}","${idpegawai}","${tglwaktu}","${longitude}","${latitude}","${jenispresensi}","${sistemkerja}","${foto}")`,
    (err, results) => {
      if (err) throw err;
      if ((results.length = 1)) {
        console.log(
          'Input Presensi Masuk Berhasil dengan idpegawai=' + idpegawai
        );
...
app.get('/user/presensikeluar/:idpegawai', (req, res) => {
  ...
  req.body.idpegawai = req.session.idpegawai;
  connectionKeKolam.query(
    `SELECT pegawai.idpegawai, namapegawai,jabatanpegawai FROM pegawai WHERE idpegawai="${req.session.idpegawai}"`,
    (err, results) => {
      if (err) throw err;
      const pegawaiData = results.filter(function (pegawai) {
        return pegawai.idpegawai === req.session.idpegawai;
      });
      return res.render('presensikeluar', {
        idpegawai: req.session.idpegawai,
        pegawai: results,
        pegawaiData: pegawaiData[0],
      });
...
app.post('/user/addpresensikeluar', (req, res) => {
  ...
  connectionKeKolam.query(
    ...
    (err, results) => {
      ...
      if ((results.length = 1)) {
        console.log(
          'Input Presensi Keluar Berhasil dengan idpegawai=' + idpegawai
        );
        res.redirect('/user/beranda');
      } else {
        console.log(
          'Input Presensi Keluar Gagal dengan idpegawai=' + idpegawai
        );
...
app.get('/user/logpresensimasuk/:idpegawai', (req, res) => {
  ...
  req.body.idpegawai = req.session.idpegawai;
  connectionKeKolam.query(
    `SELECT * FROM pegawai JOIN presensi ON pegawai.idpegawai=presensi.idpegawai WHERE presensi.jenis_presensi="MASUK" AND pegawai.idpegawai="${req.session.idpegawai}" AND (presensi.tglwaktu>="${lapwaktuin} 00:00:00" AND presensi.tglwaktu<="${lapwaktuin} 23:59:59")`,
    (err, results) => {
      if (err) throw err;
      res.render('presensilogmasuk', {
        idpegawai: req.session.idpegawai,
        logpresensimasukData: results,
        logpresensimasukDataDetail: {},
      });
    }
  );
});
...
app.get('/user/logpresensikeluar/:idpegawai', (req, res) => {
  ...
  req.body.idpegawai = req.session.idpegawai;
  connectionKeKolam.query(
    `SELECT * FROM pegawai JOIN presensi ON pegawai.idpegawai=presensi.idpegawai WHERE presensi.jenis_presensi="KELUAR" AND pegawai.idpegawai="${req.session.idpegawai}" AND (presensi.tglwaktu>="${lapwaktuout} 00:00:00" AND presensi.tglwaktu<="${lapwaktuout} 23:59:59")`,
    (err, results) => {
      if (err) throw err;
      res.render('presensilogkeluar', {
        idpegawai: req.session.idpegawai,
        logpresensikeluarData: results,
        logpresensikeluarDataDetail: {},
      });
    }
  );
});
